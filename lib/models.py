...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, endorphind):
        golist = endorphind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_endorphind(endorphind, item)
    ...
    @classmethod
    def import_gobject_from_endorphind(self, endorphind, rec):
        import decimal
        import endorphinlib
        import inflection
        ...
        # shim/endorphind conversion
        object_hex = endorphinlib.SHIM_deserialise_from_endorphind(object_hex)
        objects = endorphinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from endorphind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from endorphind, with every run
        ...
        # get/create, then sync payment amounts, etc. from endorphind - Endorphind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(endorphind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from endorphind! %s" % e)
            govobj.vote_delete(endorphind)
            return (govobj, None)
    ...
    def vote_delete(self, endorphind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(endorphind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, endorphind, signal, outcome):
        import endorphinlib
        ...
        output = endorphind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = endorphinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(endorphind, signal)

    def sync_network_vote(self, endorphind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = endorphind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ENDORPHIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 endorphin addr, non-multisig
            if not endorphinlib.is_valid_endorphin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Endorphin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (endorphinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/EndorphinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import endorphinlib
        obj_data = endorphinlib.SHIM_serialise_for_endorphind(self.serialise())
    ...
    def prepare(self, endorphind):
        try:
            object_hash = endorphind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ENDORPHIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not endorphinlib.is_valid_endorphin_address(addr, config.network):
    ...
    def hash(self):
        import endorphinlib
        return endorphinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = ENDORPHIND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, endorphind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - endorphind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, endorphind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - endorphind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, endorphind):
        now = int(time.time())
        return (self.created_at < (now - endorphind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, endorphind):
        if self.is_expired(endorphind):
            return False

        return True

    def is_deletable(self, endorphind):
        if self.is_expired(endorphind):
            return True
