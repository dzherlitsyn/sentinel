...
def is_valid_endorphin_address(address, network='mainnet'):
    ...
    endorphin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Endorphin address
        return False

    if (address_version != endorphin_version):
        return False
...
# shims 'til we can fix the endorphind side
def SHIM_serialise_for_endorphind(sentinel_hex):
    from models import ENDORPHIND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = ENDORPHIND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in endorphind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # endorphind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    endorphind_hex = serialise(obj)
    return endorphind_hex


# shims 'til we can fix the endorphind side
def SHIM_deserialise_from_endorphind(endorphind_hex):
    from models import ENDORPHIND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(endorphind_hex)

    # shim from endorphind
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('endorphin.conf')
