...
class EndorphinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some endorphind methods
    ...
    @classmethod
    def from_endorphin_conf(self, endorphin_dot_conf):
        from endorphin_config import EndorphinConfig
        config_text = EndorphinConfig.slurp_config_file(endorphin_dot_conf)
        creds = EndorphinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = endorphinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = endorphinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = endorphinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_endorphind_proto_version_with_sentinel_ping)
