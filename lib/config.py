...
sentinel_cfg = EndorphinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_endorphind_proto_version_with_sentinel_ping = 70207


def get_endorphin_conf():
    home = os.environ.get('HOME')

    endorphin_conf = os.path.join(home, ".endorphincore/endorphin.conf")
    if sys.platform == 'darwin':
        endorphin_conf = os.path.join(home, "Library/Application Support/EndorphinCore/endorphin.conf")

    endorphin_conf = sentinel_cfg.get('endorphin_conf', endorphin_conf)

    return endorphin_conf
...
def get_db_conn():
    ...
    db_name = sentinel_cfg.get('db_name', 'sentinelendorphin')
    db_user = sentinel_cfg.get('db_user', 'sentinelendorphin')
    db_password = sentinel_cfg.get('db_password', 'sentinelendorphin')
...
endorphin_conf = get_endorphin_conf()
network = get_network()
db = get_db_conn()
