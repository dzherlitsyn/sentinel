...
class GovernanceClass(object):
    ...
    def vote(self, endorphind, signal, outcome):
        return self.go.vote(endorphind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, endorphind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(endorphind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(endorphind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to endorphind "gobject list" output
    ...
    def endorphind_serialise(self):
        import endorphinlib
        return endorphinlib.SHIM_serialise_for_endorphind(self.serialise())
