...
def has_endorphin_conf():
    import config
    import io

    valid_endorphin_conf = False

    # ensure endorphin_conf exists & readable
    #
    # if not, print a message stating that Endorphin Core must be installed and
    # configured, including JSONRPC access in endorphin.conf
    try:
        f = io.open(config.endorphin_conf)
        valid_endorphin_conf = True
    except IOError as e:
        print(e)

    return valid_endorphin_conf
...
def main():
    ...
    if not has_endorphin_conf():
        print("EndorphinCore must be installed and configured, including JSONRPC access in endorphin.conf")
        sys.exit(1)
